...
public class PoiExcelUtil {
   ...
   */
  public static String write2FilePath(HSSFWorkbook hssfWorkbook, String basePath) {
    // 文件名
    ...
    LogUtil.debug(PoiExcelUtil.class, "【将生成的Excel输出到指定目录】basePath ：" + basePath);
   ...
   * @param path         文件输出目录
   */
  public static void write2File(HSSFWorkbook hssfWorkbook, String path, String name) {
    // 生成目录
    File store = new File(path);
    if (!store.exists()) {
      store.mkdirs();
    }
    LogUtil.debug(PoiExcelUtil.class, "【将生成的Excel输出到指定文件】path ：" + path);
    FileOutputStream fileOut = null;
    try {
      fileOut = new FileOutputStream(path + File.separator + name);
      hssfWorkbook.write(fileOut);
    } catch (Exception e) {
      throw new RuntimeException("将生成的Excel输出到指定目录失败");
    } finally {
  ...
  public static <T> String createToFilePath(String title, String path, String[] headers, List<T> dataList, Map<String, Function<T, String>> formats) {
    ...
    LogUtil.debug(PoiExcelUtil.class, "【生成Excel,并存放到指定文件夹目录下】title : {} , path : {} , headers : {}", title, path, headers);
    ...
    // 5.保存文件到filePath中
    return write2FilePath(hssfWorkbook, path);
  }
  ...
  public static <T> void createToFilePath(String sheetName, String title, String filePath, String[] mainDataFields, Object mainData, String[] headers,
    ...
    if (ArrayUtils.isEmpty(headers)) {
      LogUtil.debug(PoiExcelUtil.class, "【参数headers为空】");
      throw new IllegalArgumentException("headers");
    }
    if (ArrayUtils.isEmpty(mainDataFields)) {
      LogUtil.debug(PoiExcelUtil.class, "【参数mainDataFields】");
      throw new IllegalArgumentException("mainDataFields");
    }
    if (mainData == null) {
      LogUtil.debug(PoiExcelUtil.class, "【参数mainData】");
  ...
  public static <T> HSSFWorkbook createExcel2Export(String sheetName, String title, String[] headers, List<T> dataList, Map<String, Function<T, String>> formats) {
    if (ArrayUtils.isEmpty(headers)) {
      LogUtil.debug(PoiExcelUtil.class, "【表头为空】");
      throw new RuntimeException("表头不能为空");
    }
  ...
  public static List<String[]> readExcel(String excelPath, int skipRows, int columnCount, int sheetNo, Set<Integer> columnNumberForSkipValueValidateSet,
    ...
    if (StringUtils.isBlank(excelPath)) {
      LogUtil.debug(PoiExcelUtil.class, "【参数excelPath为空】");
    ...
    try {
      ...
      LogUtil.error(PoiExcelUtil.class, e, "【Excel解析失败】");
